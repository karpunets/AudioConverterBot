    ...
    public void handle(Update update, AudioConverterBot bot) {
        ...
        try {
            ...
            if (telegramUser.isReadyToConvert()) {
                ...
                bot.execute(new SendMessage(message.getChatId(), "Now send me a voice or audio"));
            }

            telegramUserRepository.save(telegramUser);
        } catch (IllegalArgumentException e) {
            bot.execute(new SendMessage(message.getChatId(), "Unfortunately, I will only understand the formats of supported audio conversion"));
        }
    }
}
